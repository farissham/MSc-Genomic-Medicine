...
include { COLOC_SUSIE_ANALYSIS } from '../subworkflows/local/coloc_susie'
...
workflow SUMSTATS {
    ...
    ch_coloc_susie_summary = Channel.empty()
    if ('coloc_susie' in mods) {
        if (!params.coloc_susie_loci || !params.coloc_susie_ref_type) {
            error "COLOC_SUSIE: --coloc_susie_loci and --coloc_susie_ref_type (quant|cc) are required for the 'coloc_susie' module"
        }
        if (!params.coloc_susie_ref && !params.coloc_susie_ref_dir) {
            error "COLOC_SUSIE: provide --coloc_susie_ref (eQTL file) or --coloc_susie_ref_dir (pre-filtered gene dir)"
        }
        if (params.coloc_susie_auto_genes && !params.coloc_susie_ref) {
            error "COLOC_SUSIE: --coloc_susie_auto_genes requires --coloc_susie_ref (full eQTL file)"
        }
        if (!params.genotype) {
            error "COLOC_SUSIE: --genotype (PLINK prefix) is required for the 'coloc_susie' module (shared LD for both GWAS and eQTL side — see bin/coloc_susie.R)"
        }
        def coloc_susie_loci_ch    = file(params.coloc_susie_loci, checkIfExists: true)
        def coloc_susie_ref_ch     = params.coloc_susie_ref     ? file(params.coloc_susie_ref,     checkIfExists: true) : null
        def coloc_susie_ref_dir_ch = params.coloc_susie_ref_dir ? file(params.coloc_susie_ref_dir, checkIfExists: true) : null
        COLOC_SUSIE_ANALYSIS(HARMONISE.out.harmonised, coloc_susie_ref_ch, coloc_susie_ref_dir_ch,
                              PREPARE_REFERENCE.out.genotype, coloc_susie_loci_ch)
        ch_coloc_susie_summary = COLOC_SUSIE_ANALYSIS.out.summary
        ch_versions            = ch_versions.mix(COLOC_SUSIE_ANALYSIS.out.versions)
    }
    ...
    coloc_susie_summary = ch_coloc_susie_summary
    ldsc_h2        = ch_ldsc_h2
    ldsc_rg        = ch_ldsc_rg
